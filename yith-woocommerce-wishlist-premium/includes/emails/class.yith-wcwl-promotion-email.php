		...
		public function get_custom_content_html() {
			...
			$replace = array_merge(
				array(
					...
					'product_image' => apply_filters( 'yith_wcwl_promotional_email_item_thumbnail', '<div style="margin-bottom: 5px"><img src="' . ( $this->object->get_image_id() ? current( wp_get_attachment_image_src( $this->object->get_image_id(), 'full' ) ) : wc_placeholder_img_src() ) . '" alt="' . esc_attr__( 'Product image', 'woocommerce' ) . '" height="auto" width="350px" style="vertical-align:middle; margin-' . ( is_rtl() ? 'left' : 'right' ) . ': 10px;" /></div>', $this->object ),
					'product_name' => $this->object->get_title(),
					'product_price' => $this->object->get_price_html(),
					'product_url' => $this->object->get_permalink(),
					'add_to_cart_url' => esc_url( add_query_arg( 'add-to-cart', yit_get_product_id( $this->object ), $this->object->get_permalink() ) ),
...
<p>Use this coupon code <b><a href="{add_to_cart_url}">{coupon_code}</a></b> to obtain a wonderful discount!</p>', 'yith-woocommerce-wishlist' ),
                    ),
...
{coupon_code}
to obtain a wonderful discount!', 'yith-woocommerce-wishlist' ),
				)
