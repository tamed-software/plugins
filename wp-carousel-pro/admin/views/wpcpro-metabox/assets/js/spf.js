...
;(function( $, window, document, undefined ) {
  ...
  SP_WPCP.helper = {
    ...
    name_nested_replace: function( $selector, field_id ) {
      ...
      var checks = [];
      var regex  = new RegExp('('+ SP_WPCP.helper.preg_quote(field_id) +')\\[(\\d+)\\]', 'g');
  ...
  $.fn.spf_nav_metabox = function() {
    return this.each( function() {
      ...
      $links.on('click', function( e ) {
        ...
        SP_WPCP.helper.set_cookie('spf-last-metabox-tab-'+ post_id +'-'+ unique_id, section_id);
      ...
      var get_cookie = SP_WPCP.helper.get_cookie('spf-last-metabox-tab-'+ post_id +'-'+ unique_id);
    ...
    return this.each( function() {
      $(this).find('.spf--media').spf_reload_script();
        ...
        if( $preview_block.length ) {
          ...
          $this.on('change', SP_WPCP.helper.debounce( function( event ) {
    ...
    return this.each( function() {
      ...
      var interval = setInterval(function () {
        if( $this.is(':visible') ) {
          ...
          if( data_editor.theme !== 'default' && SP_WPCP.vars.code_themes.indexOf(data_editor.theme) === -1 ) {
    ...
    return this.each( function() {
      ...
      $wrapper.sortable({
        ...
        update: function( event, ui ) {
          ...
          SP_WPCP.helper.name_nested_replace( $wrapper.children('.spf-cloneable-item'), field_id );
      ...
      var event_clone = function( e ) {
        ...
        var $this           = $(this),
            ...
            cloned_regex    = new RegExp('('+ SP_WPCP.helper.preg_quote(field_id) +')\\[(\\d+)\\]', 'g');
        ...
        if( !SP_WPCP.vars.icon_modal_loaded ) {
          ...
          window.wp.ajax.post( 'spf-get-icons', { nonce: $button.data('nonce') } ).done( function( response ) {
            ...
            $load.on('click', 'a', function( e ) {
              ...
              SP_WPCP.vars.$icon_target.find('i').removeAttr('class').addClass(icon);
              SP_WPCP.vars.$icon_target.find('input').val(icon).trigger('change');
              SP_WPCP.vars.$icon_target.find('.spf-icon-preview').removeClass('hidden');
              SP_WPCP.vars.$icon_target.find('.spf-icon-remove').removeClass('hidden');
    ...
    return this.each( function() {
      ...
      $wrapper.sortable({
        ...
        update: function( event, ui ) {
          ...
          SP_WPCP.helper.name_nested_replace( $wrapper.children('.spf-repeater-item'), field_id );
          $wrapper.spf_customizer_refresh();
  ...
  $.fn.spf_shortcode = function() {
    ...
    return this.each( function() {
      ...
      $insert.on('click', function( e ) {
        ...
        var shortcode = '';
        var serialize = $modal.find('.spf-field:not(.hidden)').find(':input').serializeObjectSP_WPCP();
      ...
      $modal.on('click', '.spf--repeat-button', function( e ) {
        ...
        SP_WPCP.helper.name_nested_replace( $modal.find('.spf--repeat-shortcode'), sc_group );
  ...
  $.fn.spf_field_wp_editor = function() {
    return this.each( function() {
      ...
      var editor_on_change = function( editor ) {
        editor.on('change', SP_WPCP.helper.debounce( function() {
          editor.save();
  ...
  $.fn.spf_color = function() {
    return this.each( function() {
      ...
      var $input        = $(this),
          picker_color  = SP_WPCP.funcs.parse_color( $input.val() ),
          ...
          $transparent_slider.slider({
            ...
            create: function() {
              ...
              $container.on('click', '.wp-picker-default', function() {
                ...
                var default_color = SP_WPCP.funcs.parse_color( $input.data('default-color') ),
    ...
    return this.each( function() {
      ...
      $this.on({
        mouseenter: function() {
          ...
          offset_left = ( SP_WPCP.vars.is_rtl ) ? ( $this.offset().left + 24 ) : ( $this.offset().left - $tooltip.outerWidth() );
    ...
    return this.each( function() {
      ...
      if( $complex.length ) {
        ...
        var $input  = $complex.find(':input'),
            ...
            obj     = $input.serializeObjectSP_WPCP(),
  ...
  $.fn.spf_customizer_listen = function( options ) {
    ...
    return this.each( function() {
      ...
      $input.on('change keyup', SP_WPCP.helper.debounce( function() {
  ...
  // Window on resize
  //
  SP_WPCP.vars.$window.on('resize spf.resize', SP_WPCP.helper.debounce( function( event ) {

    var window_width = navigator.userAgent.indexOf('AppleWebKit/') > -1 ? SP_WPCP.vars.$window.width() : window.innerWidth;
  ...
  // This code for taxonomies values in post meta category area
  // $('.sp_wpcp_post_type select').change(function (event) {
  //   event.preventDefault();
  //   var data = {
  //     action: 'wpcp_get_taxonomies',
  ...
  //   }
  //   $.post(ajaxurl, data, function (resp) {
  //     $('#sp_wpcp_post_cat_taxonomy').html(resp);
  //     $("#sp_wpcp_post_cat_taxonomy").trigger("chosen:updated");
  //   });
  // });
  // // // This code for taxonomies values in post meta tag area
  // $('.sp_wpcp_post_type select').change(function (event) {
  //   event.preventDefault();
  //   var data = {
  //     action: 'wpcp_get_taxonomies',
  ...
  //   }
  //   $.post(ajaxurl, data, function (resp) {
  //     $('#sp_wpcp_post_tag_taxonomy').html(resp);
  //     $("#sp_wpcp_post_tag_taxonomy").trigger("chosen:updated");
  //   });
  // });
  // Update term list on the change of post type.
  // $('.sp_wpcp_post_type select').change(function (event) {
  //   event.preventDefault();
  //   var data = {
  //     //action: 'wpcp_get_taxonomies',
  //     action: 'wpcp_get_terms',
  ...
  //   //  event.preventDefault();
  //   $('.sp_wpcp_post_taxonomy').change( 'select' ).trigger('change');

  //     var data = {
  //       action: 'wpcp_get_terms',
  //       wpcp_post_taxonomy: $('.sp_wpcp_post_taxonomy select').val(),
  //     }
  //     console.log( data.wpcp_post_taxonomy );
  //     $.post(ajaxurl, data, function (resp) {
  //       $('.sp_wpcp_taxonomy_terms select').html(resp);
  //       $(".sp_wpcp_taxonomy_terms select").trigger("chosen:updated");
