 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'SP_WPCP_Abstract' ) ) {
	abstract class SP_WPCP_Abstract {
		...
		public function add_enqueue_google_fonts() {
			...
			if ( ! empty( $this->pre_fields ) ) {
				...
				foreach ( $this->pre_fields as $field ) {
					...
					if ( $field_type && $field_id ) {
						...
						SP_WPCP::maybe_include_field( $field_type );

						$class_name = 'SP_WPCP_Field_' . $field_type;
			...
			if ( ! empty( $this->typographies ) && empty( $this->args['async_webfont'] ) ) {
				...
				$handle = 'SP_WPCP-google-web-fonts-' . $this->unique;
		...
		public function add_output_css() {
			...
			$this->output_css = apply_filters( "SP_WPCP_{$this->unique}_output_css", $this->output_css, $this );
