...
class OsModel {
  ...
  // ADD
  public function add_error($code, $error_message = 'Campo no es válido.'){
  ...
  protected function validates_email($property){
    if(isset($this->$property) && !empty($this->$property) && OsUtilHelper::is_valid_email($this->$property)){
      ...
      return new WP_Error($property, sprintf( __( '%s no es válido', 'latepoint' ), $this->get_property_nice_name($property) ));
    }
  }
  ...
  protected function validates_presence($property){
    ...
    if($validation_result){
      ...
      return new WP_Error($property, sprintf( __( '%s no puede estar vacío', 'latepoint' ), $this->get_property_nice_name($property) ));
    }
  }
  ...
  protected function validates_uniqueness($property){
    if(isset($this->$property) && !empty($this->$property)){
      ...
      if($items){
        return new WP_Error($property, sprintf( __( '%s debe ser único', 'latepoint' ), $this->get_property_nice_name($property) ));
      } 
    }
    return true;
  }
