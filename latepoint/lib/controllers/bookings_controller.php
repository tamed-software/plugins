...
if ( ! class_exists( 'OsBookingsController' ) ) :
  ...
  class OsBookingsController extends OsController {
    ...
    function __construct(){
      ...
      $this->vars['page_header'] = __('Reuniones', 'latepoint');
      $this->vars['breadcrumbs'][] = array('label' => __('Reuniones', 'latepoint'), 'link' => OsRouterHelper::build_link(OsRouterHelper::build_route_name('Reuniones', 'pending_approval') ) );
    }



    public function pending_approval(){
      $this->vars['page_header'] = __('Reuniones pendientes', 'latepoint');
      $this->vars['breadcrumbs'][] = array('label' => __('Reuniones pendientes', 'latepoint'), 'link' => false );
    ...
    public function index(){
      ...
      if(isset($this->params['download']) && $this->params['download'] == 'csv'){
        ...
        $labels_row = [  __('ID', 'latepoint'), 
                              ...
                              __('Experto', 'latepoint'), 
                              __('Telefono del experto', 'latepoint'), 
    ...
    */
    ...
    public function login_customer(){
      ...
      if($customer){
        ...
        $status = LATEPOINT_STATUS_ERROR;
        $response_html = __('Lo sentimos algo anda mal con los datos ingresados.', 'latepoint');
      }
      if($this->get_return_format() == 'json'){
      ...
      if($logged_in_customer && $this->params['booking']['customer_id']){
        ...
        if($logged_in_customer->id == $this->params['booking']['customer_id']){
          ...
          if($logged_in_customer->email != $customer_params['email']){
            ...
            if($customer_with_email_exist){
              $status = LATEPOINT_STATUS_ERROR;
              $response_html = __('Otro cliente esta registrado con este email.', 'latepoint');
            }
          }
        }else{
          // LOGGED IN BUT WRONG CUSTOMER ID PASSED VIA FORM
          $status = LATEPOINT_STATUS_ERROR;
          $response_html = __('Cliente no existe', 'latepoint');
        }
      }else{
        // NEW REGISTRATION
        $customer = new OsCustomerModel();
        ...
        if($customer_exist){
          ...
          if($customer_exist->can_login_without_password()){
            ...
            $status = LATEPOINT_STATUS_ERROR;
            $response_html = __('Una cuenta con este email existe actualmente. Porfavor intenete nuevamente.', 'latepoint');
          }
        }
      }
