...
if($agents){ ?>
	<?php if(!$calendar_only){ ?>
		<div class="page-header-main-actions">
			<label><?php _e('Calendario del mes', 'latepoint'); ?></label>
	...
	if(!$calendar_only) echo '<div class="calendar-month-agents-w '.$calendar_not_scrollable_class.'" data-route="'.OsRouterHelper::build_route_name('bookings', 'monthly_agents').'">';
		echo '<div class="ma-floated-days-w">';
			echo '<div class="ma-head-info"><span>'.__('Fecha', 'latepoint').'</span><span>'.__('Expero', 'latepoint').'</span></div>';
					    ...
					    list($work_start_minutes, $work_end_minutes) = OsBookingHelper::get_work_start_end_time_for_date(['custom_date' => $calendar_start_date->format('Y-m-d'), 'agent_id' => $agent->id]);
							...
							echo '<div class="ma-day-agent-bookings">';
								if($total_work_time > 0){
									...
									echo '<div class="ma-day-off"><span>'.__('Dias fuera', 'latepoint').'</span></div>';
								}
							echo '</div>';
						}
					echo '</div>';
  ...
  <div class="no-results-w">
    ...
    <h2><?php _e('Sin expertos creados', 'latepoint'); ?></h2>
    <a href="<?php echo OsRouterHelper::build_link(OsRouterHelper::build_route_name('agents', 'new_form') ) ?>" class="latepoint-btn"><i class="latepoint-icon latepoint-icon-plus-square"></i><span><?php _e('Crear experto', 'latepoint'); ?></span></a>
  </div>
<?php } ?>
