<div class="step-confirmation-w latepoint-step-content">
  <h3 class="confirmation-header"><?php _e('Gracias por agendar una reunión con nuestro equipo de expertos.', 'latepoint'); ?></h3>
  <div class="confirmation-number"><?php _e('Número de confirmación:', 'latepoint'); ?> <strong><?php echo $booking->id; ?></strong></div>
  <a href="<?php echo $booking->ical_download_link; ?>" class="ical-download-btn" target="_blank"><i class="latepoint-icon latepoint-icon-calendar"></i><span><?php _e('Agregar a mi calendario', 'latepoint'); ?></span></a>
  <div class="confirmation-info-w">
  	<div class="confirmation-app-info">
		  <h5><?php _e('Información de la reunión', 'latepoint'); ?></h5>
		  <ul>
        ...
        <?php if(!OsSettingsHelper::is_on('steps_hide_agent_info')){ ?>
  		  	<li><?php _e('Experto:', 'latepoint'); ?> <strong><?php echo $booking->agent->full_name; ?></strong></li>
        <?php } ?>
  ...
  <?php if($customer->is_guest && (OsSettingsHelper::get_settings_value('steps_hide_registration_prompt') != 'on')){ ?>
    ...
    <div class="info-box text-center">
    	<?php _e('¿Sabía que puede crear una cuenta para administrar sus reservas y programar nuevas reuniones?', 'latepoint'); ?>
    	<div class="info-box-buttons">
    		<a href="#" class="show-set-password-fields"><?php _e('Crear cuenta', 'latepoint'); ?></a>
    	</div>
    </div>
  <?php } ?>
</div>