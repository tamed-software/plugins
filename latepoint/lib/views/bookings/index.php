          ...
          <thead>
            ...
            <tr>
              ...
              <?php if(!$show_single_agent){ ?>
                <th><?php echo OsFormHelper::select_field('filter[agent_id]', false, OsAgentHelper::get_agents_list(), '', ['placeholder' => __('Expertos', 'latepoint'), 'class' => 'os-table-filter']); ?></th>
              <?php } ?>
              <th><?php echo OsFormHelper::text_field('filter[customer]', __('Buscar por cliente', 'latepoint'), '', ['class' => 'os-table-filter']); ?></th>
              <th><?php echo OsFormHelper::select_field('filter[status]', false, OsBookingHelper::get_statuses_list(), '', ['placeholder' => __('Todos los estados', 'latepoint'), 'class' => 'os-table-filter']); ?></th>
              <th>
                <div class="os-form-group">
                  <div class="os-date-range-picker os-table-filter-datepicker" data-single-date="yes" data-can-be-cleared="yes" data-no-value-label="<?php _e('Filtrar fecha', 'latepoint'); ?>" data-clear-btn-label="<?php _e('resetear busqueda por fecha', 'latepoint'); ?>">
                    <span class="range-picker-value"><?php _e('Filtrar fecha', 'latepoint'); ?></span>
                    <i class="latepoint-icon latepoint-icon-chevron-down"></i>
