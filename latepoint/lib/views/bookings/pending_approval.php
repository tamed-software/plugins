  ...
  <div class="latepoints-list">
      ...
      <div class="appointment-box-large status-<?php echo $booking->status; ?>" data-booking-id="<?php echo $booking->id; ?>" <?php echo OsBookingHelper::quick_booking_btn_html($booking->id); ?>>
        <div class="appointment-info">
          ...
          <div class="appointment-status-selector" data-booking-id="<?php echo $booking->id; ?>" data-route="<?php echo OsRouterHelper::build_route_name('bookings', 'change_status') ?>">
            <?php echo OsFormHelper::select_field('booking[status]', __('Estado:', 'latepoint'), OsBookingHelper::get_statuses_list(), $booking->status, ['id' => 'booking_status_'.$booking->id]); ?>
          </div>
        </div>
        <div class="account-info-w">
          <div class="account-info-head">
            ...
            <div class="account-name-w">
              <div class="account-info-label"><?php _e('Expero', 'latepoint'); ?></div>
              <div class="account-name"><?php echo $booking->agent->full_name; ?></div>
            </div>
          </div>
          <div class="account-info">
            <div class="account-property">
              <span class="label"><?php _e('Teléfono: ', 'latepoint'); ?></span>
        ...
        <div class="account-info-w">
          <div class="account-info-head">
            ...
            <div class="account-name-w">
              <div class="account-info-label"><?php _e('Cliente', 'latepoint'); ?></div>
        ...
        <div class="appointment-box-actions">
          <div class="aba-button-w aba-approve" data-status="<?php echo LATEPOINT_BOOKING_STATUS_APPROVED; ?>">
            <i class="latepoint-icon latepoint-icon-check"></i><span><?php _e('Aprovar', 'latepoint'); ?></span>
          </div>
          <div class="aba-button-w aba-reject" data-status="<?php echo LATEPOINT_BOOKING_STATUS_CANCELLED; ?>">
            <i class="latepoint-icon latepoint-icon-x"></i><span><?php _e('Rechazar', 'latepoint'); ?></span>
          </div>
        </div>
      </div>
    <?php
    endforeach; ?>
      ...
      <div class="os-pagination-w">
        <div class="pagination-info"><?php echo sprintf(__('Mostrar reuniones %d de %d hasta %d total', 'latepoint'), $showing_from, $showing_to, $total_bookings); ?></div>
        <ul>
          <?php 
...
<?php }else{ ?>
  <div class="no-results-w">
    ...
    <h2><?php _e('NO hay reuniones pendientes', 'latepoint'); ?></h2>
    <a href="#" <?php echo OsBookingHelper::quick_booking_btn_html(); ?> class="latepoint-btn"><?php _e('Crear Reunión', 'latepoint'); ?></a>
  </div>
<?php } ?>