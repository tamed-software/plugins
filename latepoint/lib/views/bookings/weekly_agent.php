<?php if($agents){ ?>
	<div class="calendar-week-agent-w" data-calendar-action="<?php echo OsRouterHelper::build_route_name('bookings', 'weekly_agent'); ?>">
		<div class="calendar-controls">
			<div class="cc-agent-selector">
				<label for=""><?php _e('Experto:', 'latepoint'); ?></label>
		...
		<div class="calendar-self-w">
			<?php if(($work_start_minutes < $work_end_minutes) && ($timeblock_interval > 0)){ 
				...
				<div class="calendar-hours">
					<div class="ch-hours">
						<div class="ch-info">
							<span><?php _e('Fecha', 'latepoint'); ?></span>
							<span><?php _e('Hora', 'latepoint'); ?></span>
						</div>
			...
			<?php }else{ ?>
			  <div class="no-results-w">
			    ...
			    <h2><?php _e('Parece que todavía no has fijado tus horas de trabajo.', 'latepoint'); ?></h2>
			    <a href="<?php echo OsRouterHelper::build_link(OsRouterHelper::build_route_name('settings', 'general')); ?>" class="latepoint-btn"><i class="latepoint-icon latepoint-icon-plus-square"></i><span><?php _e('Configurar horas de trabajo', 'latepoint'); ?></span></a>
			  </div>
			<?php } ?>
		</div>
	</div>
<?php }else{ ?>
  ...
  <div class="no-results-w">
    ...
    <h2><?php _e('No se encontraron expertos', 'latepoint'); ?></h2>
    <a href="<?php echo OsRouterHelper::build_link(OsRouterHelper::build_route_name('expertos', 'new_form') ) ?>" class="latepoint-btn"><i class="latepoint-icon latepoint-icon-plus"></i><span><?php _e('Agregar el primer experto', 'latepoint'); ?></span></a>
  </div>
<?php } ?>