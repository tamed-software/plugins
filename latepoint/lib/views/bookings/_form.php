  ...
  <form action="" data-os-action="<?php echo ($booking->is_new_record()) ? OsRouterHelper::build_route_name('bookings', 'create') : OsRouterHelper::build_route_name('bookings', 'update'); ?>" class="booking-quick-edit-form">
    <div class="os-form-header">
      <?php if($booking->is_new_record()){ ?>
        <h2><?php _e('Nueva reunión', 'latepoint'); ?></h2>
      <?php }else{ ?>
        <h2><?php _e('Editar reunión #', 'latepoint'); ?><?php echo $booking->id; ?></h2>
      <?php } ?>
    </div>
    <div class="os-form-content">
      <div class="os-form-group">
        <label for=""><?php _e('Servicio', 'latepoint'); ?></label>
        <div class="os-services-select-field-w">
          ...
          <?php }else{ ?>
            <div class="service-option-selected">
              ...
              <span><?php _e('Seleccionar servicio','latepoint'); ?></span>
            </div>
          <?php } ?>
        </div>
      </div>
      <div class="agent-info-w <?php echo ($booking->agent_id) ? 'selected': 'selecting'; ?>">
        <div class="os-form-sub-header-actions">
          <a href="#" class="latepoint-btn latepoint-btn-sm latepoint-btn-white agent-info-change-agent-btn">
            <i class="latepoint-icon latepoint-icon-refresh-cw"></i><span><?php _e('Cambiar', 'latepoint'); ?></span>
          </a>
        </div>
        <div class="agents-selector-w">
          <div class="os-form-group">
            <select name="booking[agent_id]" class="os-form-control">
              <option value=""><?php _e('Seleccionar experto', 'latepoint'); ?></option>
      ...
      <div class="os-row">
        <div class="os-col-6">
          <?php echo OsFormHelper::text_field('booking[start_date]', __('Fecha incial', 'latepoint'), $booking->start_date); ?>
        </div>
        <div class="os-col-6">
          <?php echo OsFormHelper::select_field('booking[status]', __('Estado', 'latepoint'), OsBookingHelper::get_statuses_list(), $booking->status, array('placeholder' => 'Set Status')); ?>
        </div>
      </div>
      <div class="os-row">
        <div class="os-col-6">
          <?php echo OsFormHelper::text_field('booking[start_time]', __('Hora inicial', 'latepoint'), $booking->start_time); ?>
          <?php echo OsFormHelper::text_field('booking[end_time]', __('Hora final', 'latepoint'), $booking->end_time); ?>
        </div>
        <div class="os-col-6">
          <?php echo OsFormHelper::text_field('booking[buffer_before]', __('Tiempo antes', 'latepoint'), $booking->buffer_before); ?>
          <?php echo OsFormHelper::text_field('booking[buffer_after]', __('Tiempo extra', 'latepoint'), $booking->buffer_after); ?>
        </div>
      </div>
      ...
      <div class="customer-info-w <?php echo ($booking->customer->id) ? 'selected': 'editing'; ?>">
        <div class="os-form-sub-header-actions">
          <a href="#" class="latepoint-btn latepoint-btn-sm latepoint-btn-white customer-info-edit-btn" data-os-params="customer_id=<?php echo $booking->customer->id; ?>" data-os-output-target=".customer-quick-edit-form-w" data-os-action="<?php echo OsRouterHelper::build_route_name('bookings', 'customer_quick_edit_form'); ?>">
            <i class="latepoint-icon latepoint-icon-edit-3"></i><span><?php _e('Editar', 'latepoint'); ?></span>
          </a>
          <a href="#" class="latepoint-btn latepoint-btn-sm latepoint-btn-white customer-info-create-btn" data-os-output-target=".customer-quick-edit-form-w" data-os-action="<?php echo OsRouterHelper::build_route_name('bookings', 'customer_quick_edit_form'); ?>">
            <i class="latepoint-icon latepoint-icon-plus"></i><span><?php _e('Nuevo', 'latepoint'); ?></span>
          </a>
          <a href="#" class="latepoint-btn latepoint-btn-sm latepoint-btn-white customer-info-load-btn">
            <i class="latepoint-icon latepoint-icon-search"></i><span><?php _e('Encontrar', 'latepoint'); ?></span>
          </a>
        </div>
        <div class="os-form-sub-header"><h3><?php _e('Cliente', 'latepoint'); ?></h3></div>
        <div class="customers-selector-w">
          <div class="customers-selector-search-w">
            <i class="latepoint-icon latepoint-icon-search"></i>
            <input type="text" class="customers-selector-search-input" placeholder="<?php _e('Inicia excribiendo para buscar...', 'latepoint'); ?>">
    ...
    <div class="os-form-buttons">
      <?php 
      echo '<button type="submit" class="latepoint-btn">'.__('Grabar cambios', 'latepoint').'</button>';
      echo '<a href="#" class="latepoint-btn latepoint-btn-secondary">'.__('Cancelar', 'latepoint').'</a>';
