<div class="latepoint-booking-form-element <?php if(!$show_next_btn) echo 'hidden-buttons'; ?> latepoint-color-<?php echo OsSettingsHelper::get_booking_form_color_scheme(); ?>">
  ...
  <div class="latepoint-form-w">
    <form class="latepoint-form"
      data-selected-label="<?php _e('Seleccionaste', 'latepoint'); ?>"
      ...
      <div class="latepoint-footer">
        <a href="#" class="latepoint-btn latepoint-btn-white latepoint-prev-btn disabled"><i class="latepoint-icon-arrow-2-left"></i> <span><?php _e('Atrás', 'latepoint'); ?></span></a>
        <a href="#" onclick="document.getElementsById('form_booking_tamed').style.margin = '15% 0 0 0'" class="latepoint-btn latepoint-btn-primary latepoint-next-btn <?php echo ($show_next_btn) ? '' : 'disabled'; ?>" data-pre-last-step-label="<?php _e('Confirmar', 'latepoint'); ?>" data-label="<?php _e('Continuar', 'latepoint'); ?>"><span><?php _e('Continuar', 'latepoint'); ?></span> <i class="latepoint-icon-arrow-2-right"></i></a>
      </div>
    </form>
  </div>
  <div class="latepoint-summary-w">
    ...
    <div class="os-summary-lines">
      <?php
      $selectable_values = ['service' => ['label' => __('Estas seleccionando', 'latepoint'), 'value' => $booking->service->name ],
                            'agent' => ['label' => __('Experto', 'latepoint'), 'value' => $booking->agent->full_name ],
      ...
      foreach($selectable_values as $key => $selectable_value){ ?>
        <div class="os-summary-line <?php if(!empty($selectable_value['value'])) echo 'os-has-value'; ?>">
          ...
          <div class="os-summary-value os-summary-value-<?php echo $key; ?>">
            <input id="label-<?php $id_agente = str_replace(" ", "_", $selectable_value['value']); echo $id_agente; ?>" type="hidden" value="<?php echo $id_agente; ?>">
