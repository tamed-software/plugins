  ...
  <form action="" data-os-success-action="redirect" data-os-redirect-to="<?php echo OsRouterHelper::build_link(OsRouterHelper::build_route_name('services', 'index')); ?>" data-os-action="<?php echo $service->is_new_record() ? OsRouterHelper::build_route_name('services', 'create') : OsRouterHelper::build_route_name('services', 'update'); ?>">
    ...
    <div class="os-row">
      ...
      <div class="os-col-lg-6">
        <div class="white-box">
          <div class="white-box-header">
            <div class="os-form-sub-header"><h3><?php _e('Duración', 'latepoint'); ?></h3></div>
          </div>
          <div class="white-box-content">
            <?php echo OsFormHelper::text_field('service[duration]', __('Duración del servicio', 'latepoint'), $service->duration); ?>
            <div class="os-row">
              <div class="os-col-6">
                <?php echo OsFormHelper::text_field('service[buffer_before]', __('Tiempo adicional antes', 'latepoint'), $service->buffer_before); ?>
              </div>
              <div class="os-col-6">
                <?php echo OsFormHelper::text_field('service[buffer_after]', __('Tiempo adicional despues', 'latepoint'), $service->buffer_after); ?>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="white-box">
      <div class="white-box-header">
        <div class="os-form-sub-header"><h3><?php _e('Expertos', 'latepoint'); ?></h3></div>
      </div>
      <div class="white-box-content">
        ...
        <?php if($agents){
          foreach($agents as $agent){
            ...
            if($locations_count == count($locations)){
              $locations_count_string = __('Todo', 'latepoint');
            }else{
              $locations_count_string = $service->is_new_record() ? __('Todo', 'latepoint') : $locations_count.'/'.count($locations);
            }
            ?>
            ...
            <div class="agent <?php echo $active_class; ?>">
              <div class="agent-i selector-trigger">
                ...
                <div class="selected-services" data-all-text="<?php echo __('Todo', 'latepoint'); ?>">
                  <strong><?php echo $locations_count_string; ?></strong> 
                  <span><?php echo  __('Seleccionar ubicación', 'latepoint'); ?></span>
                </div>
                <a href="#" class="customize-agent-service-btn"><i class="latepoint-icon latepoint-icon-ui-46"></i><span><?php echo __('Personalizar', 'latepoint'); ?></span></a>
              </div><?php
              if($locations){ ?>
                <div class="agent-services-list-w">
                  <h4><?php echo sprintf(__('Seleccionar servicios %s que serán ofrecidos con este servicio:', 'latepoint'), $agent->first_name); ?></h4>
    ...
    <div class="white-box">
      <div class="white-box-header">
        <div class="os-form-sub-header">
          <h3><?php _e('Horario de Servicio', 'latepoint'); ?></h3>
          <div class="os-form-sub-header-actions">
            <?php echo OsFormHelper::checkbox_field('is_custom_schedule', __('Configurar Horario personalizado', 'latepoint'), 'on', $is_custom_schedule, array('data-toggle-element' => '.custom-schedule-wrapper')); ?>
          </div>
        </div>
      </div>
      <div class="white-box-content">
    ...
    <?php if(!$service->is_new_record()){ ?>
        ...
        <div class="white-box">
          <div class="white-box-header">
            <div class="os-form-sub-header"><h3><?php _e('Dias con fechas modificadas', 'latepoint'); ?></h3></div>
          </div>
          <div class="white-box-content">
            <div class="latepoint-message latepoint-message-subtle"><?php _e('El Experto comparte las programaciones diarias personalizadas que configura en la configuración general de su empresa, sin embargo, puede agregar días adicionales con horas personalizadas que serán específicas para este servicio únicamente.', 'latepoint'); ?></div>
            <?php OsWorkPeriodsHelper::generate_days_with_custom_schedule(['service_id' => $service->id]); ?>
          </div>
        </div>
        <div class="white-box">
          <div class="white-box-header">
            <div class="os-form-sub-header"><h3><?php _e('Dias festivos y dias libres', 'latepoint'); ?></h3></div>
          </div>
          <div class="white-box-content">
            <div class="latepoint-message latepoint-message-subtle"><?php _e('El experto utiliza los mismos días festivos que estableció en la configuración general de su empresa, sin embargo, puede agregar días festivos adicionales para este servicio aquí.', 'latepoint'); ?></div>
    ...
    <?php 
      if($service->is_new_record()){
        echo OsFormHelper::button('submit', __('Agregar servicio', 'latepoint'), 'submit', ['class' => 'latepoint-btn']); 
      }else{
        ...
        echo '<a href="#" class="latepoint-btn latepoint-btn-danger remove-service-btn" style="margin-left: auto;" 
                data-os-prompt="'.__('¿Estás seguro de que quieres eliminar este servicio? Se eliminarán todas las citas asociadas con él. También puede cambiar el estado a deshabilitado si desea deshabilitarlo temporalmente.', 'latepoint').'" 
                ...
                data-os-action="'.OsRouterHelper::build_route_name('services', 'destroy').'">'.__('Eliminar servicio', 'latepoint').'</a>';
      }
